...
print(
"""
This is LongestIsoforms_HOIHO_HYDVU_PTYFL.py
Usage: LongestIsoforms_HOIHO_HYDVU_PTYFL.py input.fa output.fa
Note: should work with any file with headers beginning by an Augustus-type AN (space-delimited fields)
"""
)
...
def fasta_parse(fasta_file): #when given a fasta file, the function yields tuples of (header, sequence)
	with open(fasta_file) as f:
		...
		for header in fasta_iterator:
			...
			yield (header_string, sequence, gene_name) #since we start on a >, 2 __next__ actions mean we go back on a > line
