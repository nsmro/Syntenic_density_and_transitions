...
print(
"""
This is LongestIsoforms_SCHME.py
Usage: LongestIsoforms_SCHME.py input.fa input.gff output.fa
"""
)
...
search_string_gID = 'Parent=(SMESG\w+\.\d+)'
search_string_protID = 'ID=(SMEST\w+\.\d+)'
...
def fasta_parse(fasta_file): #when given a fasta file, the function yields tuples of (header, sequence)
	with open(fasta_file) as g:
		...
		for header in fasta_iterator: 
			...
			gene_ID = d_gene_ID[header_string]
			yield (header_string, sequence, gene_ID) 
